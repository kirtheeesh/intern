          ...
          <Card>
            <CardContent className="p-6 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold">{stats?.newCount || 0}</h3>
              </div>
          ...
          <Card>
            <CardContent className="p-6 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold">{stats?.selectedCount || 0}</h3>
              </div>
          ...
          <Card>
            <CardContent className="p-6 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold">{stats?.rejectedCount || 0}</h3>
              </div>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="p-0">
            {listLoading ? (
              ...
              <div className="p-12 text-center text-slate-500">No registrations found matching your criteria.</div>
            ) : (
                  ...
                  <TableBody>
                    {registrations?.map((reg) => (
                      <TableRow key={reg.id} className="hover:bg-slate-50/50">
                        <TableCell className="font-mono text-xs text-slate-500">{reg.registrationNumber}</TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-1">
                            <a href={`tel:+91${reg.mobileNumber}`} className="text-sm text-primary hover:underline flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        ...
                        <TableCell>
                          <div className="text-sm truncate max-w-[200px]" title={reg.college || ''}>{reg.college || '-'}</div>
                          <div className="text-xs text-slate-500">{reg.degree} {reg.yearOfStudy ? `(${reg.yearOfStudy})` : ''}</div>
                        </TableCell>
                        <TableCell>
                          <Badge variant={
                            reg.status === 'New' ? 'default' :
                            reg.status === 'Selected' ? 'secondary' :
                            reg.status === 'Rejected' ? 'destructive' : 'outline'
                          } className={reg.status === 'Selected' ? 'bg-green-100 text-green-800 hover:bg-green-200' : ''}>
                            {reg.status}
                          </Badge>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button variant="ghost" size="sm" onClick={() => setSelectedRegId(reg.id)}>
                            View
                          </Button>
                        </TableCell>
                      </TableRow>
...
function ApplicantDetailModal({ registration, isOpen, onClose, onUpdated }: { registration: any, isOpen: boolean, onClose: () => void, onUpdated: () => void }) {
  const { toast } = useToast();
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0 overflow-hidden">
          ...
          <div className="flex justify-between items-start pr-6">
            <div>
              ...
              <DialogDescription className="text-slate-500 font-mono mt-1">
                Ref: {registration.registrationNumber} • Applied on {format(new Date(registration.submittedAt), "PPP")}
              </DialogDescription>
            </div>
            <Badge variant="outline" className="text-base px-3 py-1 bg-white">{registration.status}</Badge>
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <a href={`mailto:${registration.emailId}`} className="font-medium text-primary hover:underline">{registration.emailId}</a>
                </div>
                <div>
                  <span className="text-slate-500 block">Phone</span>
                  <a href={`tel:+91${registration.mobileNumber}`} className="font-medium text-primary hover:underline">+91 {registration.mobileNumber}</a>
                </div>
                <div>
                  <span className="text-slate-500 block">DOB / Gender</span>
                  <span className="font-medium">{registration.dateOfBirth} • {registration.gender}</span>
                </div>
                <div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="font-medium">{registration.degree} • {registration.yearOfStudy}</span>
                </div>
                <div>
            ...
            <section>
              ...
              <div className="space-y-4 text-sm">
                <div>
                  ...
                  <p className="bg-slate-50 p-3 rounded border text-slate-700 whitespace-pre-wrap">{registration.whyJoin}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="font-medium">{registration.canAttendAll === "Yes" ? "All 15 Days" : "Partial"} • {registration.preferredTimeSlot || "Any time"}</span>
                  </div>
                </div>
              </div>
            </section>
          ...
          <div className="space-y-6 bg-slate-50 p-5 rounded-xl border border-border h-fit">
            <div>
              <h4 className="font-bold mb-3 flex items-center gap-2"><FileText className="w-4 h-4" /> Documents</h4>
              <div className="space-y-2">
                <Button variant="outline" className="w-full justify-start" asChild disabled={!registration.resumePath}>
                  <a href={registration.resumePath ? `/api/uploads/${registration.resumePath.split('/').pop()}` : '#'} target="_blank" rel="noopener noreferrer">
                    <FileText className="w-4 h-4 mr-2" />
                    View Resume
                  </a>
                </Button>
                <Button variant="outline" className="w-full justify-start" asChild disabled={!registration.photoPath}>
                  <a href={registration.photoPath ? `/api/uploads/${registration.photoPath.split('/').pop()}` : '#'} target="_blank" rel="noopener noreferrer">
            ...
            <div className="pt-4 border-t border-border">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-medium text-slate-500 mb-1 block">Update Status</label>
                ...
                <Button 
                  className="w-full" 
                  onClick={handleSave} 
                  disabled={updateMutation.isPending || (status === registration.status && notes === (registration.notes || ""))}
                >
